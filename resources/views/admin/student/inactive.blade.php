...
@section('content')
    ...
    <div class="row my-3">
        ...
        <div class="col-md-2">
            ...
            <select class="form-control select2" id="sreni_id" name="sreni_id" style="width: 100%;" required>
                <option selected="selected" value="">Select Class</option>
                @foreach ($srenis as $sreni)
                    <option {{ $sreni->id == old('sreni_id') ? 'selected' : '' }} value="{{ $sreni->id }}">
                        {{ $sreni->name }}
                    </option>
                @endforeach
            </select>
    ...
    <script>
        $(document).ready(function() {
            ...
            var table = $("#product").DataTable({
                ...
                ajax: {
                    url: "{{ route('students.inactive') }}",
                    data: function(d) {
                        d.from_date = $('#from_date').val();
                        d.to_date = $('#to_date').val();
                        d.bibag_id = $('#bibag_id').val();
                        d.sreni_id = $('#sreni_id').val();
                    }
                },
            ...
            $('#export_excel').click(function(e) {
                ...
                let sreniId = $('#sreni_id').val();
                let url = "{{ route('students.export.excel') }}" + "?from_date=" + fromDate + "&to_date=" +
                    toDate + "&bibag_id=" + bibagId + "&sreni_id=" + sreniId;
